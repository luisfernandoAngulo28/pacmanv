...
string CSMemoria::mostrar_exam() {
	string cab = "ocupado  | DIR |DATO|    ID      | LINK |,";
	//string lin = "+" + string(cab.length() - 10, '-') + "+,";
	string out = cab;
	int edir = 3, edat = 8, eid = 10; // eid lo cambian
	string id ="";//= string(rel, ' ') + mem[i].id;
	...
	for (int i = 0; i < MAX; i++) {
		...
		string ocup = !dir_libre(i) ? "ocupado  " : "no cupado";
		rel = edir - to_string(i).length();
		string dir = " " + string(rel, '0') + to_string(i) + " ";
		rel = edat - to_string(mem[i].dato).length();
		string dat = string(rel, ' ') + to_string(mem[i].dato);
		rel = eid - mem[i].id.length();
	   ...
	   /*	for (int s = 0; s < rel; s++) {
		...
		id=" "+mem[i].id;
		for (int s = mem[i].id.length() ; s+1 < eid; s++) {
		id+=" ";
		}
		//string id = string(rel, ' ') + mem[i].id;
		rel = edir - to_string(mem[i].link).length();
...
string CSMemoria:: get_linea (int f) {
	int i=0;
	int j=0;
	string fila = "";
	string pal = mostrar_exam();
	int c =pal.length();
